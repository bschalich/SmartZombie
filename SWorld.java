...
/**
 * Scrolling World class courtesy of danpost: http://www.greenfoot.org/scenarios/6719
 * 
...
public class SWorld extends World
{
    ...
    public void setMainActor(Actor main, int xRange, int yRange)
    {
        ...
        super.addObject(main, getWidth()/3 , getHeight()/2);
        mainActor = main;
        xRange=(int)Math.min(xRange, getWidth());
        yRange=(int)Math.min(yRange, getHeight());
        actorMinX=(2*getWidth())/7-xRange/2;
        actorMaxX=(3*getWidth())/8+xRange/2;
        actorMinY=getHeight()/2-yRange/4;
        actorMaxY=getHeight()/2+yRange/4;
    }
